 ...
 * @readonly
 */
const playersList: HTMLDivElement = document.getElementById('players-list') as HTMLDivElement;
 ...
 * @readonly
 */
const form: HTMLFormElement = document.getElementById('select-ranking') as HTMLFormElement;
...
form.addEventListener('submit', (e: SubmitEvent) => {
  ...
  if (submitter.classList.contains('clicked')) return;

  (document.getElementById('select-ranking-btn') as HTMLButtonElement).classList.add('clicked');
 ...
 * @listens click
 */
(document.getElementById('colorMode') as HTMLButtonElement).addEventListener('click', () => {
  toggleColorMode();
});
...
function toggleColorMode(): void {
  if (document.documentElement.classList.contains('black')) {
    chrome.storage.local
      .set({ darkMode: false })
      .then(() => document.documentElement.classList.remove('black'));
  } else {
    chrome.storage.local
      .set({ darkMode: true })
      .then(() => document.documentElement.classList.add('black'));
  }
}
 ...
 * @returns {void}
 */
function displayPlayersList(): void {
  playersList.innerHTML = '';

  chrome.storage.local.get('players', (data: { players: string[] }) => {
    playersList.innerHTML = '';
...
function sendListenMessage(): void {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    if (!(activeTab.url as string).match('habbocity.fr')) return;

    chrome.tabs.sendMessage(activeTab.id as number, 'listenClick');
  });
}
